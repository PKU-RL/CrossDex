...
class PPO:
    ...
    def run(self):
        ...
        if self.is_testing:
            ...
            #print(n_successes_per_env / n_dones_per_env)
            ...
            if hasattr(self.vec_env, "hand_names"):
                ...
                print()
            # success per object
            if hasattr(self.vec_env, "object_fns"):
                import matplotlib.pyplot as plt
                success_per_object = {}
                N = len(self.vec_env.object_fns)
                for i, fn in enumerate(self.vec_env.object_fns):
                    success_per_object[fn.split('.')[0]] = \
                ...
                plt.xticks(rotation=45)  # 如果物体名称较长，可以旋转横轴标签
                plt.subplots_adjust(bottom=0.5)
                plt.savefig('success_per_object.png')
            ...
            #         id = (id+1)%self.vec_env.max_episode_length
            #         # Compute the action
            #         actions = self.actor_critic.act_inference(current_obs)
            #         # Step the vec_environment
