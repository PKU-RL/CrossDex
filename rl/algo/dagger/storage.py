...
class RolloutStorage:
    ...
    def __init__(self, num_envs, buffer_size, obs_shape, states_shape, actions_shape, device='cpu', sampler='sequential',
        ...
        if pred_arm_dof_pos:
            self.arm_dof_pos_diff = torch.zeros(buffer_size, num_envs, 6, device=self.device)

    def add_transitions(self, observations, actions, rewards, dones, arm_dof_pos_diff=None):
        ...
        if self.pred_arm_dof_pos:
            self.arm_dof_pos_diff[self.step].copy_(arm_dof_pos_diff)
