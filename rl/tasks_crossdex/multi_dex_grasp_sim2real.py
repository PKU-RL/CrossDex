...
class MultiDexGraspSim2real(VecTask):
    def __init__(
        self,
        cfg,
        rl_device,
        ...
        print("\n##################################### Initializing MultiDexGraspSim2real task")
        ...
        self.force_sensor_tensor = gymtorch.wrap_tensor(force_sensor_tensor).view(self.num_envs, -1)
        self.gym.refresh_force_sensor_tensor(self.sim)
        self.init_table_force_z = self.force_sensor_tensor[:, 2].clone()
        ...
        # dr: object perception bias
        self.episode_object_pos_bias = torch.rand((self.num_envs, 3), dtype=torch.float32).to(self.device) * self.dr_object_pos_bias
        random_angle = torch.rand((self.num_envs), dtype=torch.float32).to(self.device) * self.dr_object_rot_bias
        random_axis = torch.rand((self.num_envs, 3), dtype=torch.float32).to(self.device)
        self.episode_object_quat_bias = quat_from_angle_axis(random_angle, random_axis)

        # save arm dof pos
        self.arm_dof_pos_buf = torch.zeros(
            (self.num_envs, self.num_arm_dofs[0]), device=self.device, dtype=torch.float)
        self.arm_dof_pos_diff_buf = torch.zeros(
            (self.num_envs, self.num_arm_dofs[0]), device=self.device, dtype=torch.float)
    ...
    def init_configs(self, cfg):
        ...
        # domain randomization on object perception
        dr_object_perception = self.cfg["task"]["object_perception"]
        self.dr_object_pos_bias = dr_object_perception["pos_bias"]
        self.dr_object_rot_bias = dr_object_perception["rot_bias"]
        self.dr_object_pos_noise = dr_object_perception["pos_noise"]
        self.dr_object_rot_noise = dr_object_perception["rot_noise"]
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # add force sensor on table
        sensor_pose = gymapi.Transform()
        self.gym.create_asset_force_sensor(table_asset, 0, sensor_pose)
        ...
        for i in range(num_envs):
            ...
            if "screenshot" in self.cfg:
                #self.gym.set_light_parameters(self.sim, 0, gymapi.Vec3(1,1,1), gymapi.Vec3(0.1,0.1,0.1), gymapi.Vec3(1,1,2))
                #self.gym.set_light_parameters(self.sim, 1, gymapi.Vec3(0.33,0.33,0.33), gymapi.Vec3(0.1,0.1,0.1), gymapi.Vec3(1,0,1))
                #self.gym.set_light_parameters(self.sim, 2, gymapi.Vec3(0.33,0.33,0.33), gymapi.Vec3(0.1,0.1,0.1), gymapi.Vec3(1,1,1))
                self.gym.set_rigid_body_texture(env_ptr, table_actor, 0, gymapi.MESH_VISUAL, self.table_texture_handle)
                self.gym.set_rigid_body_texture(env_ptr, side_panel_actor, 0, gymapi.MESH_VISUAL, self.table_texture_handle)
                #table_color = [210/255, 180/255, 140/255]
                #self.gym.set_rigid_body_color(env_ptr, table_actor, 0, gymapi.MESH_VISUAL, gymapi.Vec3(*table_color))
                #self.gym.set_rigid_body_color(env_ptr, side_panel_actor, 0, gymapi.MESH_VISUAL, gymapi.Vec3(*table_color))
                ...
                #hand_color = [50/255, 50/255, 50/255]
                #hand_rigid_body_index = [i for i in range(7, self.num_robot_bodies[i_robot])]
                #for o in hand_rigid_body_index:
                #    self.gym.set_rigid_body_color(env_ptr, robot_actor, o, gymapi.MESH_VISUAL, gymapi.Vec3(*hand_color))
    ...
    def _prepare_robot_asset(self, asset_root, asset_files):
        ...
        self.agg_hand_dof_indices, self.agg_arm_dof_indices, self.agg_robot_dof_indices, self.per_robot_dof_indices = [], [], [], []
        c=0
    ...
    def reset_idx(self, env_ids):
        ...
        rot_z = np.random.uniform(-self.reset_rotation_noise, self.reset_rotation_noise, len(env_ids))
        self.root_state_tensor[self.object_indices[env_ids], 5] = torch.tensor(np.cos(rot_z),dtype=torch.float32,device=self.device)
        self.root_state_tensor[self.object_indices[env_ids], 6] = torch.tensor(np.sin(rot_z),dtype=torch.float32,device=self.device)
        ...
        # dr: object perception bias
        self.episode_object_pos_bias[env_ids] = torch.rand((len(env_ids), 3), dtype=torch.float32).to(self.device) * self.dr_object_pos_bias
        random_angle = torch.rand(len(env_ids), dtype=torch.float32).to(self.device) * self.dr_object_rot_bias
        random_axis = torch.rand((len(env_ids), 3), dtype=torch.float32).to(self.device)
        self.episode_object_quat_bias[env_ids] = quat_from_angle_axis(random_angle, random_axis)
        #print(self.episode_object_pos_bias, self.episode_object_quat_bias)

        # save arm dof pos
        self.arm_dof_pos_buf[env_ids] = self.robot_dof_pos[:, self.agg_arm_dof_indices].reshape(self.num_envs, self.num_arm_dofs[0])[env_ids].clone()
    ...
    def post_physics_step(self):
        ...
        # save arm dof pos
        self.arm_dof_pos_diff_buf[:] = (self.robot_dof_pos[:, self.agg_arm_dof_indices].reshape(self.num_envs, self.num_arm_dofs[0]) \
            - self.arm_dof_pos_buf[:]).clone()
        self.arm_dof_pos_buf[:] = self.robot_dof_pos[:, self.agg_arm_dof_indices].reshape(self.num_envs, self.num_arm_dofs[0]).clone()
        #print(self.arm_dof_pos_diff_buf)
    ...
    def compute_required_observations(self, obs_buf, obs_type, num_obs):
        ...
        # dr: add object perception noise
        if self.randomize:
            self.object_pos_noisy = self.object_pos + self.episode_object_pos_bias + \
                torch.rand((self.num_envs, 3), dtype=torch.float32).to(self.device) * self.dr_object_pos_noise
            random_angle = torch.rand(self.num_envs, dtype=torch.float32).to(self.device) * self.dr_object_rot_noise
            random_axis = torch.rand((self.num_envs, 3), dtype=torch.float32).to(self.device)
            object_quat_noise = quat_from_angle_axis(random_angle, random_axis)
            self.object_rot_noisy = quat_mul(object_quat_noise, quat_mul(self.episode_object_quat_bias, self.object_rot))
            self.object_pose_noisy = torch.cat([self.object_pos_noisy, self.object_rot_noisy], dim=-1)
            #print(self.object_pos_noisy, self.object_rot_noisy, self.object_pose_noisy)
        else:
            self.object_pos_noisy = self.object_pos
            self.object_rot_noisy = self.object_rot
            self.object_pose_noisy = self.object_pose
        ...
        if "objpose" in obs_type: # object pose: pos, rot (7)
            obs_buf[:, obs_end: obs_end+7] = self.object_pose_noisy
            obs_end += 7
        ...
        if "objpcl" in obs_type: # object point cloud
            self.sampled_pcl = self.transform_obj_pcl_2_world(noisy=True)
            #self.ground_truth_pcl = self.transform_obj_pcl_2_world(noisy=False)
    ...
    def transform_obj_pcl_2_world(self, noisy=True):
        if noisy:
            o2w_pos = self.object_pos_noisy[:,:3].clone()
            o2w_quat = self.object_rot_noisy.clone()
        else:
    ...
    def compute_reward(self, actions):
        (
            ...
            ### 10-17: table force sensor
            table_force_z = self.force_sensor_tensor[:,2],
            init_table_force_z = self.init_table_force_z,
            ### 9-8: forced lift test
