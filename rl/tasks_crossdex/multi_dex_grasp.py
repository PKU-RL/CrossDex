    ...
    def compute_required_observations(self, obs_buf, obs_type, num_obs):
        ...
        if "keypts" in obs_type: # keypoints: palm pos + 4 fingertip pos, 5*3
            ...
            for i in range(self.num_robots):
                ...
                ## 11-23 revised: padded to 4 fingers
                n_f = min(self.num_fingers[i], 4)
                ft_pos = self.fingertip_pos[:, offset:offset+n_f].reshape(self.agg_num_envs,1,n_f,3)
                ft_pos = torch.cat([ft_pos, torch.zeros(self.agg_num_envs,1,4-n_f,3).to(self.device)], dim=2)
                keypos.append(ft_pos)
